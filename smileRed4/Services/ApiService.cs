...
namespace smileRed4.Services
{
    public class ApiService
    {
        ...
        internal Task GetList1<T>(string v1, string v2, string v3)
        {
        ...
        public async Task<NumberRequest> GetEmailExists(
           string urlBase,
           string servicePrefix,
           string controller,
           string email)
        {
            try
            {
                ...
                var result = await response.Content.ReadAsStringAsync();
                return JsonConvert.DeserializeObject<NumberRequest>(result);
            }
            catch
            {
                return null;
